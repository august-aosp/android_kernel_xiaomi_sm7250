...
#endif
{
	...
	if (task_placement_boost_enabled(p) || fbt_env.need_idle ||
	    prefer_high_cap || is_rtg ||
#ifdef CONFIG_SCHED_WALT
			__cpu_overutilized(prev_cpu, delta) ||!task_fits_max(p, prev_cpu) || cpu_isolated(prev_cpu)) {
#else
			cpu_overutilized(prev_cpu) || !task_fits_max(p, prev_cpu)) {
#endif
		best_energy_cpu = cpu;
		goto unlock;
	}
...
#else
		want_affine = !wake_wide(p, 1) && cpumask_test_cpu(cpu, &p->cpus_allowed);
#endif
	}
